...
for (const title of ["Searchable catalog", "Contacts CRUD", "Support ticket"])
  test(`clean room demo: ${title}`, async ({ page }) => {
    ...
    try {
      ...
      await page
        ...
        .click();
      jobId = (await (await started).json()).id;
      ...
      const archive = await page.request.get(
        (await page
          .getByRole("link", { name: "Download app + evidence", exact: true })
      ...
      if (jobId) {
        await expect
          ...
          .not.toBe("running");
        const closed = await page.request.delete(`/api/clean-room?id=${jobId}`);
